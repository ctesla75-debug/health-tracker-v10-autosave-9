...
async function getLogCount(){
  // More reliable than getAll() on some WebView/PWA contexts
  if(USE_LOCAL_STORAGE_FALLBACK){
    return lsLoadAll().length;
  }
  let db;
  try{ db = await openDB(); }
  catch(e){ USE_LOCAL_STORAGE_FALLBACK=true; return getLogCount(); }
  return new Promise((resolve,reject)=>{
    try{
      const tx = db.transaction(STORE,"readonly");
      ...
      // if count() isn't available for some reason, fallback to getAllLogs
      getAllLogs().then(arr=>resolve(arr.length)).catch(reject);
    }
  });
}
...
async function updateRecordCount(){
  // Use a dedicated count() call so this shows correctly after reopening the PWA
  const n = await getLogCount();
  $("#recordCount").textContent = `${n} record${n===1?"":"s"} stored`;
...
function openDeleteAllModal(){
  ...
  $("#modalDeleteAll").setAttribute("aria-hidden","false");
  setTimeout(()=> $("#deleteAllText").focus(), 0);
}
function closeDeleteAllModal(){
  $("#modalDeleteAll").classList.remove("show");
  $("#modalDeleteAll").setAttribute("aria-hidden","true");
}
...
function attachDirtyTracking(){
  ...
  const skipIds = new Set([
    ...
    "clearFrom","clearTo",
    "importEmptyRows",
    // delete-all modal controls (should not trigger autosave)
    "deleteAllText"
  ]);
  inputs.forEach(el=>{
...
window.addEventListener("load", async ()=>{
  ...
  // delete all (advanced)
  $("#btnDeleteAllOpen").addEventListener("click", openDeleteAllModal);
  $("#btnCancelDeleteAll").addEventListener("click", closeDeleteAllModal);
  $("#modalDeleteAll").addEventListener("click", (e)=>{
    if(e.target?.dataset?.close === "1") closeDeleteAllModal();
  });
  $("#btnConfirmDeleteAll").addEventListener("click", async ()=>{
    const typed = ($("#deleteAllText").value || "").trim().toUpperCase();
    if(typed !== "DELETE"){
  ...
  // Exit button (web apps cannot forcibly close; provide clear instructions)
  $("#btnExit").addEventListener("click", ()=>{
    alert("To exit: press your phone's Home/Back button (or close the tab).\n\nYour data is already saved automatically.");
  });
